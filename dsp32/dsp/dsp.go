...
// over a is returned.
// For an empty input an empty output is returned.
func MedianFilter(a []float32, width int) []float32 {
	if width >= len(a) {
		width = len(a)
	}
...
// Repeat makes a slice of float32 of length n and sets all values to x. If n <= 0
...
func Reciprocal(a []float32) []float32 {
	b := make([]float32, len(a))
	for i := range b {
		b[i] = 1.0 / a[i]
	}
...
func SafeReciprocal(a []float32, replacement float32) []float32 {
	b := make([]float32, len(a))
	for i := range b {
		if a[i] == 0 {
			b[i] = replacement
...
func Resample(a []float32, newLen int) []float32 {
	if len(a) == 0 || newLen <= 0 {
		return nil
	}
	if len(a) == 1 {
	...
	for i := 1; i < len(b)-1; i++ {
		...
		b[i] = float32(fraction*float64(a[low]) + (1.0-fraction)*float64(a[high]))
	}
	return b
}
